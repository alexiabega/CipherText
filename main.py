...
def main():
    ...
    while ask:
        ...
        if text == 'e':
            ...
            ciphertext_unfiltered = open("Cipher.txt")
            ciphertext_unfiltered1 = ciphertext_unfiltered.read()

            # Filters the text so it is only alphanumeric characters, and lowercase
            ciphertext = ''.join(x.lower() for x in ciphertext_unfiltered1 if x.isalpha())
